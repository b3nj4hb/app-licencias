...
import { contribuyente, localidad, persona, plocalidad } from 'src/app/classes/classes';
...
export class ViewVoucherComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.listarlocalidad(1)
  }

  listarlocalidad(id:number) {
    this.perService.buscarlocalidad(id).subscribe(data => {
      this.localidad = data
      console.log(data)
  ...
  listarcontribuyentes() {
    this.perService.listarcontribuyentes().subscribe(data => {
